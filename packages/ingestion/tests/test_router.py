...
def test_pdf_extension_routes_to_pdf_source(tmp_path) -> None:
    p = tmp_path / "spec.pdf"
    p.write_bytes(b"%PDF-1.4 dummy")  # not parsed here, just routed
    assert isinstance(source_for_path(p), PdfSource)


def test_text_extension_routes_to_text_source_with_file_tag(tmp_path) -> None:
    p = tmp_path / "notes.md"
...
def test_unknown_extension_still_read_as_text(tmp_path) -> None:
    p = tmp_path / "data.log"
